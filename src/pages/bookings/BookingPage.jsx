  ...
  const handleCancelBooking = (booking) => {
   ...
   const handleMarkPaid = (booking) => {
    setConfirmDialog({
      open: true,
      title: 'Mark Paid',
      content: `Are you sure you want to mark the payment paid for ${booking.name}?`,
      action: 'Paid',
      booking
    });
  };
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      const payload = {
        paymentId: booking.paymentId,
        bookingId: booking.bookingId,
        action: action==="Paid"?"UpdatePayment":'UpdateBooking',
        status: action==="Paid"?"Offline":action,
      ...
      if (response.ok && result?.response?.code === 1001) {
        fetchBookings();
        setSnackbar({
          open: true,
        ...
        throw new Error(result?.responseStatus?.message || 'Unknown error');
      }
    } catch (err) {
    ...
    <Grid container spacing={3}>
      ...
      >
              ...
              <Grid item xs={12}>
                ...
                <Paper variant="outlined" sx={{ p: 3, borderRadius: 2 }}>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <Typography variant="subtitle1" fontWeight="bold">AppointmentNo ID</Typography>
                      <Typography variant="body1">{detailsDialog.booking.appointmentNo}</Typography>
                    </Grid>
                  </Grid>
                </Paper>
              </Grid>
              ...
              <Grid item xs={12}>
                ...
                <Paper variant="outlined" sx={{ p: 3, borderRadius: 2 }}>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <Typography variant="subtitle1" fontWeight="bold">Payment Method</Typography>
                      <Typography variant="body1">{detailsDialog.booking.paymentMode || 'N/A'}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
        ...
        <DialogActions sx={{ px: 4, pb: 4, pt: 2 }}>
          ...
          {detailsDialog.booking && detailsDialog.booking.paymentStatus !== 'Paid' && detailsDialog.booking.status !== 'Cancelled' && (
            <Button
              onClick={() => {
                handleCloseDetailsDialog();
                console.log("detailsDialog",detailsDialog)
                handleMarkPaid(detailsDialog.booking);
              }}
              color="primary"
              variant="contained"
              size="large"
