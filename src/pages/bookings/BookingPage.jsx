...
export default function BookingPage() {
  ...
  const [loading, setLoading] = useState(true); // Initial load set to true
  const [searchLoading, setSearchLoading] = useState(false); // Separate state for search button
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.responseStatus?.message || 'Failed to update booking');
      }

      if (!data.responseStatus || data.responseStatus.code !== 1000) {
        throw new Error(data.responseStatus?.message || 'Invalid booking status');
      }
        ...
        b.bookingId === booking.bookingId
          ? { 
              ...
              status: action === 'cancel' ? 'Cancelled' : 
                     action === 'approve' ? 'Approved' : b.status,
      ...
      setSnackbar({
        open: true,
        message: action === 'approve-payment' ? 'Payment approved successfully' :
                 action === 'approve' ? 'Booking approved successfully' :
  ...
  return (
    <Grid container spacing={3}>
      ...
      >
        {/* Confirm dialog content remains the same 
      </Dialog>
    ...
    try {
      ...
      const response = await fetch(
        'https://7n0wver1gl.execute-api.eu-west-2.amazonaws.com/dev/fetch-bookings-by-date',
        {
          ...
          body: JSON.stringify({
            date: formattedDate,
            ...(selectedStatus && { status: selectedStatus === 'Accepted' ? 'Approved' : selectedStatus }),
            ...(consultantID && { consultantID }),
            pharmacyNo,
          })
        }
        ...
        const transformedBookings = data.bookings
          ...
          .map((booking) => ({
            ...
            paymentStatus: booking.paymentStatus ? 
              (booking.paymentStatus.toUpperCase() === 'PAID' ? 'Paid' : 'Pending') : 
              'Pending',
            status: booking.status === 'Approved' ? 'Accepted' : booking.status || 'Booked',
            userID: booking.userID || '',
            pharmacyNo: booking.consultantID || pharmacyNo,
            type: booking.type,
            paymentId: booking.paymentId || '',
...
export default function BookingPage() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      // Handle both 1000 (success) and 1001 (no bookings found) as valid responses
